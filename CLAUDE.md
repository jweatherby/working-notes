...
- **Publish:** force-pushes `dist/plugin` to the `dist` branch, then creates GitHub release `v<version>` with the zip (for Claude desktop Chat, which only takes uploads).
- **marketplace.json** points the plugin at that branch: `"source": {"source": "github", "repo": …, "ref": "dist", "path": "plugin"}`. So Claude Code, Cowork and `bun run setup` all install the released plugin; a development machine's MCP server still runs its clone (shim order above).
