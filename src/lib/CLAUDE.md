...
| `relation/components/AddRelation` | `self` (`RelationEnd` from `relationEnd()` in `$shared/utils/relations`), `onDone` | Kind select ("Related to", "Depends on", "Needed by"; the inverse swaps the ends via `toRelationInput`), then a `SearchPicker` over `loadEntityOptions` (`$shared/trpc/load-entity-options`) scoped by `ENTITY_SEARCH_SCOPES` (`$shared/utils/entity`). Picking a target adds the link. No note field for now |
